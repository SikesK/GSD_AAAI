...
def domain_to_string_for_cost_based_init_state(domain: Domain, action_str: str) -> str:
    """Print a PDDL domain object."""
    result = f"(define (domain {domain.name})"
    body = ""
    indentation = " " * 4
    ...
    body += _sort_and_print_collection(
        ...
        domain.actions,
        "",
        to_string=lambda obj: obj.cost_str() + "\n",
    )
    result = result + "\n" + indent(body, indentation) + "\n)"
    result = _remove_empty_lines(result)
    return result

def domain_to_string_for_arbitrary_action_str(domain: Domain, action_str: str) -> str:
    """Print a PDDL domain object."""
    result = f"(define (domain {domain.name})"
    body = ""
    indentation = " " * 4
...
def problem_to_string_for_cost_based_init_state(problem: Problem) -> str:
    ...
    body += f"{'(:goal ' + str(problem.goal) + ')'}\n(:metric minimize (total-cost))" if problem.goal != TRUE else ""
    result = result + "\n" + indent(body, indentation) + "\n)"
    result = _remove_empty_lines(result)
    return result
