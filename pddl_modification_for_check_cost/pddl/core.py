...
class Action:
    ...
    def cost_str(self):
        ...
        operator_str += f"    :parameters ({_typed_parameters(self.parameters)})\n"
        if self.precondition is not None:
            if type(self.precondition) == Predicate:
                new_precondition = "(and " + str(self.precondition) + ")"
            else:
                new_precondition = "(and "+ " ".join([str(i) for i in self.precondition.operands]) + ")"
            operator_str += f"    :precondition {new_precondition}\n"
        if self.effect is not None:
            if type(self.effect) == Predicate or type(self.effect) == Not:
               operator_str += f"    :effect {str(self.effect)}\n"
            else:
               operator_str += "    :effect (and " + " ".join([str(i) for i in self.effect.operands]) + " (increase (total-cost) 1)) \n"
        operator_str += ")"
        return operator_str
